...
function cpuStress {
    ...
    echo -e "$(date -u) >>> CPUJ1: Running for $stress_time seconds at $stress_cpu_load load" | tee -a $cpu_log $verbose_log
    # Log time and load for CPUJ2
    stress_timer_freq=$(shuf -i 1000-50000 -n 1)
    stress_timer_ops=$(shuf -i 3600000-7200000 -n 1)
    echo -e "$(date -u) >>> CPUJ2: Running for $stress_timer_ops ops at $stress_timer_freq load" | tee -a $cpu_log $verbose_log
