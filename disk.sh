...
function makeFile {
    ...
    echo -e "$(date -u) >>> DJ$disk_base_id/$disk_sub_id created $make_file_name.temp sized $make_file_size MB, will sleep $sleep_timer seconds" | tee -a $disk_log $verbose_log
    sleep $sleep_timer
